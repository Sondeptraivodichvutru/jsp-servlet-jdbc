...
<c:url var="APIurl" value="/api-admin-deleted-product" />
						...
						<div class="table-data__tool">
							...
							<div class="table-data__tool-right">
								<a flag="info"
									...
									href='<c:url value="/admin-product?type=list&page=1&maxPageItem=6&status=1&sortName=name&sortBy=desc"/>'> <span>
										<i class="zmdi"></i>Back
								</span>
								</a>
								 <a id="btnDelete" class="au-btn au-btn-icon au-btn--green au-btn--small" type="button"
                                                           data-toggle="tooltip" title='Refund'>
                                                            <span> <i class="zmdi"></i>Refund
                                                            </span>
                                                        </a>
									...
									<c:forEach var="item" items="${model.listResult}">
										<tr class="tr-shadow">
											<td><input type="checkbox" id="checkbox_${item.id}"
												value="${item.id}"></td>
											<td>${item.name}</td>
											<td><c:forEach var="cate" items="${categories}">
		...
		$j
				.ajax({
					...
					success : function(result) {
						window.location.href = "${ProductURL}?type=list&maxPageItem=6&status=1&page=1&message=delete_success";
					},
					error : function(error) {
						window.location.href = "${ProductURL}?type=list&maxPageItem=6&status=1&page=1&message=error_system";
					}
				});
	}
</script>
